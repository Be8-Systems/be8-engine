...
export default class Be8 {
    ...
    async setup () {
        ...
        databaseGroupKeys.forEach(({ groupID, version, groupKey }) => this.#groupKeys.set(`${groupID}:${version}`, groupKey));
    ...
    async addPublicKeys (publicKeys = []) {
        ...
        const proms = publicKeys.map(function ({ accID, publicKey }) {
            ...
            publicKeysStore.onsuccess = () => console.log(`engine: added public key for ${accID}`);
        });
    ...
    async addGroupKeys (groupID, groupKeys) {
        if (groupID && groupKeys?.length > 0) {
            ...
            const proms = groupKeys.map(function ({ version, groupKey }) {
                ...
                groupKeysStore.onsuccess = () => console.log(`engine: added group key for ${groupID}`);
            });
    
            return await Promise.all(proms);
        } else {
            console.log(`engine: missing groupID: "${groupID}" or keys: "${groupKeys}" in addGroupKey`);
        }
    }
    ...
    async getCachedGroupKeys () {
        ...
        return await new Promise(function (success) {
            all.onsuccess = function (event) {
                const keys = event.target.result.map(key => ({ groupID: key.groupID, version: key.version, groupKey: key }));
                return success(keys);
            };
        });
    }
    ...
    async getCachedGroupVersions (groupID) {
        ...
        return await new Promise(function (success) {
            all.onsuccess = function (event) {
                ...
                const groupVersions = allKeys.filter(key => key.groupID === groupID)
                    .map(v => v.version)
                    .sort(function (a, b) {
